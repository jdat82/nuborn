...
module.exports = function(grunt) {
	...
	var scopes = { all:'all', libs:'libs', app:'app', res:'res' };
	...
	var resBuilder = require("./ResourcesBuilder");
	...
	grunt.registerMultiTask('nuborn', 'Your task description goes here.', function() {
		...
		switch (extension) {
			...
			case "html":
				if (this.data.html && this.data.html[scope]) {
					launchHTMLBuild(this.data.targets, this.data.html[scope], scope);
				} else {
		...
		switch(scope) 
		{
			case scopes.res:
				...
				if(this.data.resources.fileset) {
					resBuilder.copyStaticResources(this.data.targets, this.data.resources.fileset);
				}
	 ...
	 * -- HTML
	 */
	var launchHTMLBuild = function(targets, configuration, scope) {
		// if no target is provided, stop process
		if (!targets) {
			grunt.log.writeln("There is not any target");
			return true;
