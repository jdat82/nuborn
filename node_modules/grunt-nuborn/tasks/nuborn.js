...
module.exports = function(grunt) {
	 ...
	 */
	 var launchHTMLBuild = function(targets, configuration, targetName){
		...
		var output = configuration.output ? addSeparatorToPath(configuration.output) : "";
		...
		var templates = grunt.file.expand("src"+sep+"**"+sep+configuration.templatesFolderName+sep+"*.html");
		for(var i = 0, length = templates.length; i < length; i++){
			var file = templates[i];
			var filename = path.basename(file);
			var _path = file.replace(filename, "");
			var module = path.basename(path.dirname(path.dirname(file)));
			android && grunt.file.copy(file, androidOutput+configuration.templatesFolderName+sep+module+sep+filename);
			ios && grunt.file.copy(file, iosOutput+configuration.templatesFolderName+sep+module+sep+filename);
			web && grunt.file.copy(file, webOutput+configuration.templatesFolderName+sep+module+sep+filename);
		}
	};
