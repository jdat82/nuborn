...
// resources builder
var ResourceBuilder = require("processor/ResourceProcessor");

// html builder
var HtmlBuilder = require("processor/HtmlProcessor");

// css builder
var CssBuilder = require("processor/CssProcessor");
		...
		*/
		...
		switch (extension) 
		{
			...
			case "scss":
				...
				if (this.data.css) {
					if(scope === scopes.all) {
						cssBuilder.process(this.data.targets, this.data.css[scopes.libs], scope);
						cssBuilder.process(this.data.targets, this.data.css[scopes.app], scope);
					} else {
						cssBuilder.process(this.data.targets, this.data.css[scope], scope);
					}
				} else {
			...
			case "html":
				...
				if (this.data.html) {
					if(scope === scopes.all) {
						htmlBuilder.process(this.data.targets, this.data.html[scopes.libs], scope);
						htmlBuilder.process(this.data.targets, this.data.html[scopes.app], scope);
					} else {
						htmlBuilder.process(this.data.targets, this.data.html[scope], scope);
					}
				} else {
